...
---
## 🧪 **테스트**
처음에는 테스트코드를 꼭 작성해야 한다는 점은 알고 있었지만, 실제로 크게 체감하지는 못했습니다. 하지만 프로젝트가 진행되면서 요구사항이 늘어나고 로직이 복잡해질수록, 테스트를 작성하지 않으면 변경된 코드가 기존 기능과 잘 어우러져 동작하는지 확신할 수 없다는 걸 느끼게 되었습니다. 이로 인해 점점 코드에 대한 신뢰가 떨어지는 경험을 했고, 그래서 단위 테스트를 집중적으로 작성하기 시작했습니다.  

<img width="2062" height="732" alt="image" src="https://github.com/user-attachments/assets/fa69abf5-d88b-4c4d-8c44-5508c95ed3f0" />

단위 테스트에서는 특히 각 기능과 메서드가 의도한 대로 정확히 동작하는지를 중점적으로 검증했습니다. 또한 사용자가 예상치 못한 상황이나 돌발 행동을 할 수 있다는 점을 상상하며, 잘못된 입력값, 중복 요청, 비정상적인 상태 변화 등 다양한 시나리오에 대해 예외 처리가 제대로 이루어지는지도 꼼꼼히 테스트했습니다. 이를 통해 실사용 환경에서 발생할 수 있는 여러 문제 상황에 대해 사전에 대비하고자 했습니다.  

<img width="1920" height="504" alt="image" src="https://github.com/user-attachments/assets/a5bbd755-ccf5-4466-8a35-3d578706e21e" />


또한, JaCoCo 같은 도구를 활용해 테스트 커버리지를 측정하며 어느 정도 코드가 테스트되고 있는지 점검했고, 통합 테스트는 N+1 문제나 동시성 문제 등 특정 이슈가 발생했을 때만 제한적으로 사용했습니다. 다만, 통합 테스트의 필요성도 점차 느끼고 있어 앞으로는 더욱 적극적으로 활용할 계획입니다.  

<img width="1920" height="504" alt="image" src="https://github.com/user-attachments/assets/68fef688-81ac-4b68-a8cd-4fdec2ceca4c" />


테스트코드는 단순히 오류를 찾기 위한 수단이 아니라, 변경 사항이 기존 시스템에 미치는 영향을 최소화하고 안정적인 개발을 뒷받침하는 필수적인 과정이라고 생각합니다. 그래서 앞으로도 테스트 범위를 확장하고, 다양한 상황에 대응할 수 있도록 꾸준히 개선해 나갈 계획입니다.
